...
func (m *Map[K, V]) getInode() *inode[K, V] {
	n := (*inode[K, V])(atomic.LoadPointer(&m.inode))
	if n == nil {
		m.lock.Lock()
		n = (*inode[K, V])(atomic.LoadPointer(&m.inode))
		if n == nil {
...
func (m *Map[K, V]) getInodeAndBucket(hash uintptr) (*inode[K, V], *bucket[K, V]) {
	n := m.getInode()
	i := hash & n.mask
	b := &(n.buckets[i])
	if !b.inited() {
...
func (n *inode[K, V]) initBuckets(m *Map[K, V]) {
...
func (n *inode[K, V]) initBucket(m *Map[K, V], i uintptr) {
	b := &(n.buckets[i])
	b.lock.Lock()
	if b.inited() {
		b.lock.Unlock()
	...
	b.m = make(map[K]V)
	p := (*inode[K, V])(atomic.LoadPointer(&n.pred)) // predecessor
	if p != nil {
		if n.mask > p.mask {
			// Grow
...
func (b *bucket[K, V]) inited() bool {
	return atomic.LoadInt64(&b.init) == 1
}

func (b *bucket[K, V]) freeze() map[K]V {
	b.lock.Lock()
	b.frozen = true
	m := b.m
	b.lock.Unlock()
...
func (b *bucket[K, V]) clone() []entry[K, V] {
	b.lock.RLock()
...
func (b *bucket[K, V]) tryLoad(key K) (value V, ok bool) {
	b.lock.RLock()
	value, ok = b.m[key]
	b.lock.RUnlock()
	return
...
func (b *bucket[K, V]) tryStore(m *Map[K, V], n *inode[K, V], check bool, key K, value V) (done bool) {
	b.lock.Lock()
	if b.frozen {
		b.lock.Unlock()
		return
	...
	// Using length check existence is faster than accessing.
	l0 := len(b.m)
	b.m[key] = value
	length := len(b.m)
	b.lock.Unlock()
...
func (b *bucket[K, V]) tryDelete(m *Map[K, V], n *inode[K, V], key K) (done bool) {
	b.lock.Lock()
	if b.frozen {
		b.lock.Unlock()
		return
